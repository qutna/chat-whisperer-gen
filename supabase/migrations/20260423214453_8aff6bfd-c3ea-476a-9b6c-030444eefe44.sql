...
BEGIN
  -- Count 1: total filtered trips (uses indexes only, no join)
  SELECT COUNT(*)::bigint INTO v_total_trips
  FROM public.trips t
  ...
  -- Count 2: surveyed trips (drive from small surveys table via INNER JOIN)
  SELECT COUNT(*)::bigint INTO v_surveyed_trips
