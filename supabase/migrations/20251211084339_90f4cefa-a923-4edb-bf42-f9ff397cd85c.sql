-- Remove the overly permissive public SELECT policy on trips
DROP POLICY IF EXISTS "Anyone can view trips" ON public.trips;

-- Create a restrictive policy - only service role can read raw trip data
-- This ensures data can only be accessed through SECURITY DEFINER functions
CREATE POLICY "Only service role can view trips directly" 
ON public.trips 
FOR SELECT 
USING (false);

-- Update get_trip_aggregation to enforce minimum k-anonymity threshold of 5
...
BEGIN
  ...
  -- Return aggregated data with k-anonymity threshold (min 5 trips per group)
