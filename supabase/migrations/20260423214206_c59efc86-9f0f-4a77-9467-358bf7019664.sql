-- Strategy: Compute the total and surveyed counts using a single
-- aggregate over filtered trips, but use a hash anti-join to avoid
-- the per-row EXISTS subquery.
-- For the per-mode breakdown, drive from the small trip_surveys table
-- and look up each trip via PK.
...
BEGIN
  -- Single hash-join scan to count total and surveyed trips
  SELECT
    COUNT(*)::bigint,
